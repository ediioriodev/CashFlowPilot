...
import { formatCurrency, formatDate, getCurrentMonthRange } from "@/lib/formatUtils";
...
export default function SpesePage() {
  ...
  const handleMonthChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Otteniamo l'ultimo giorno del mese calcolando il giorno "0" del mese successivo
    ...
    <ProtectedRoute>
      <div className={clsx("min-h-screen pb-20 transition-colors duration-300", scope === 'P' ? "bg-gray-50/90" : "bg-gray-50")}>
        ...
        <main className="p-4 max-w-2xl mx-auto space-y-4">
           {/* Total Card & Filters */}
           <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 sticky top-20 z-10 flex flex-col gap-3">
              ...
              <div className="relative">
                ...
                <input 
                    ...
                    className="w-full pl-9 pr-8 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all placeholder:text-gray-400"
                />
                {searchText && (
                    <button 
              ...
              <div className="flex items-center justify-end">
                 <div className="flex items-center gap-2 text-sm text-gray-500 bg-gray-50 rounded-lg p-1">
                    <Eye className="w-4 h-4 ml-2" />
                    <select 
                        value={filterMode} 
                        onChange={(e) => setFilterMode(e.target.value as FilterMode)}
                        className="bg-transparent border-none text-xs sm:text-sm focus:ring-0 cursor-pointer text-gray-700 py-1 outline-none"
                    >
           ...
           {loading ? (
             ...
             filteredExpenses.map((expense) => {
               ...
               <div key={expense.id} className={clsx(
                   "bg-white p-4 rounded-xl shadow-sm border flex flex-col gap-3 transition-shadow relative overflow-hidden",
                   needsConfirmation ? "border-orange-200 bg-orange-50/50" : "border-gray-100"
               )}>
                    {needsConfirmation && (
                        <div className="absolute top-0 right-0 p-1 bg-orange-100 rounded-bl-lg">
                    ...
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col">
                            <span className="font-bold text-gray-800 text-lg leading-tight">{expense.negozio}</span>
                            <span className="text-xs text-gray-500 font-medium mt-1">{expense.ambito} • {formatDate(expense.data_spesa)}</span>
                            {expense.note_spese && <span className="text-xs text-gray-400 italic mt-1">{expense.note_spese}</span>}
                        </div>
                        <div className="flex flex-col items-end">
                             <span className={clsx(
                                 "font-bold text-lg",
                                 expense.tipo_transazione === 'entrata' ? "text-green-600" : "text-red-600"
                             )}>
                                 {expense.tipo_transazione === 'entrata' ? '+' : '-'} {formatCurrency(expense.importo)}
                             </span>
                             {isRecurring && (
                                <div className="flex items-center gap-1 mt-1 text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-full">
                    ...
                    {/* Actions */}
                    <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-100 border-dashed">
                        {needsConfirmation && (
                             <button 
                                onClick={() => setConfirmId(expense.id!)}
                                className="flex items-center gap-1 text-xs font-bold bg-green-100 text-green-700 px-3 py-1.5 rounded-lg hover:bg-green-200 transition-colors"
                             >
